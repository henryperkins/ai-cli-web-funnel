...
| 10. Test + CI expansion                  | all                  | medium | none             | SLO integration-db + explicit profile-e2e/ops-smoke decision in docs | AQ-050, MQ-038, DR-018 |
...
10. Step 10 is complete only when CI runs baseline checks, migration `012`/`013` contract coverage, profile install integration-db, and the profile-e2e/ops-smoke path is implemented with evidence in `DECISION_LOG.md`, `docs/ci-verification.md`, and `docs/wave9-build-report.md`.
...
12. Step 12 is complete only when `docs/wave9-build-report.md` is published with step-by-step evidence and explicit governance boundary statement.

## Step 10 Closure Decision (2026-02-28, updated)

1. Profile-specific e2e scenario is implemented (`tests/e2e/profile-lifecycle-local.e2e.test.ts`).
2. Required CI now executes profile lifecycle e2e via explicit `npm run test:e2e-local` step in `.github/workflows/forge-ci.yml`.
3. Ops smoke workflow is implemented (`.github/workflows/forge-ops-smoke.yml`) as non-blocking `workflow_dispatch` + nightly run, with fail-closed migration bootstrap (`ON_ERROR_STOP`, no error suppression).
4. Reference decisions: `DECISION_LOG.md` (`DLOG-0036`, `DLOG-0037`).
