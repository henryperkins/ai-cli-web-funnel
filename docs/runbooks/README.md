...
- [Cron Go-Live Checklist](./cron-go-live-checklist.md)
- [Security Trust-Gate Operations](./security-trust-gate-operations.md)
...
- `npm run test:integration-db:docker` (provisions ephemeral Postgres, applies migrations `001..016`, and runs `tests/integration-db`).
...
- `npm run run:security-promotion -- --mode dry-run --package-id <uuid> --reviewer-id <id> --evidence-ref <ticket-id>` (requires DB URL).
- `npm run run:beta-readiness -- --mode dry-run --from <iso> --to <iso>` (requires DB URL).
